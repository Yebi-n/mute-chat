...
EAS iOS free build quota is currently exhausted, so the immediate iOS path is Xcode Cloud. Current status:

- GitHub private repository exists: `https://github.com/Yebi-n/mute-chat`
- `ios/` was generated by GitHub Actions and committed to `main`.
- App Store Connect shows Xcode Cloud, but first workflow creation requires Xcode on macOS.

Use this order:

1. Push the repository to GitHub.
2. Run the `Generate iOS project` GitHub Actions workflow once to create and commit `ios/`.
3. Open the generated project once in macOS Xcode and create the first Xcode Cloud workflow.
4. Build the generated iOS project in Xcode Cloud and distribute to TestFlight.
...
- create the first Xcode Cloud workflow from macOS Xcode
...
2. Xcode Cloud for iOS once the first macOS Xcode setup is complete / GitHub Actions Ubuntu for Android
3. remaining EAS Build allowance after monthly reset or paid upgrade
4. GitHub Actions macOS for urgent iOS builds if Xcode Cloud setup is blocked
