...
export function displayMafiaSystemText(body: string) {
  ...
  if (normalized.startsWith("[MAFIA_CANCEL_JOIN")) return "";
  ...
  if (normalized.startsWith("[MAFIA_FINAL_DEFENSE")) {
    return `${name ?? "대상"}님이 최후의 변론을 시작합니다.`;
  }
  ...
  if (normalized.startsWith("[MAFIA_NIGHT_START")) {
    return `${day}일 차 밤이 되었습니다.`;
  }
  if (normalized.startsWith("[MAFIA_NIGHT_NO_DEATH")) {
    return "밤사이 아무도 사망하지 않았습니다.";
  }
  if (normalized.startsWith("[MAFIA_GAME_END")) {
    return get("winner") === "mafia"
      ? "마피아팀이 승리하였습니다."
      : "시민팀이 승리하였습니다.";
  }
  if (normalized.startsWith("[MAFIA_INSPECT_RESULT")) {
    return `${targetName ?? "대상"}님은 ${
      get("isMafia") === "true" ? "마피아가 맞습니다." : "마피아가 아닙니다."
    }`;
  }
  if (normalized.startsWith("[MAFIA_TIME_SHORTENED")) {
    return `${name ?? "멤버"}님이 시간을 단축하였습니다.`;
  }
  if (normalized.startsWith("[MAFIA_TIME_EXTENDED")) {
    return `${name ?? "멤버"}님이 시간을 연장하였습니다.`;
  }
