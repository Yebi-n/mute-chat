...
as $$
  with latest_claims as (
    select
      max(claim.created_at) filter (
        where claim.reward_type = 'attendance'
      ) as attendance_claimed_at,
      max(claim.created_at) filter (
        where claim.reward_type = 'rewarded_ad'
      ) as ad_claimed_at
  ...
  select
    ...
    coalesce(latest_claims.attendance_claimed_at + interval '1 hour', now()),
    latest_claims.ad_claimed_at is null
      or latest_claims.ad_claimed_at + interval '1 hour' <= now()
