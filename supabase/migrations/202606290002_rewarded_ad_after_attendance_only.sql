...
as $$
  ...
  select
    ...
    coalesce(last_attendance.claimed_at + interval '1 hour', now()) as attendance_available_at,
    case
      when last_attendance.claimed_at is null then false
      when last_attendance.claimed_at + interval '1 hour' <= now() then false
      else not exists (
        select 1
        from public.reward_claims claim
        where claim.user_id = auth.uid()
          ...
          and claim.created_at < last_attendance.claimed_at + interval '1 hour'
      )
    end as rewarded_ad_available
...
begin
  ...
  if p_reward_type = 'attendance' then
    ...
    if exists (
      ...
      where claim.user_id = auth.uid()
        ...
        and claim.created_at < v_last_attendance + interval '1 hour'
    ) then
  ...
  next_available_at := case
    ...
    else v_last_attendance + interval '1 hour'
  end;
  return next;
end;
$$;
