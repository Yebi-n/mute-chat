...
declare
  ...
  v_body text := public.mafia_display_text(p_body);
begin
...
begin
  ...
  where membership.room_id = new.room_id
    ...
    and coalesce((
      select pref.enabled
      from public.room_notification_preferences pref
      where pref.room_id = new.room_id and pref.user_id = membership.user_id
    ), true);
