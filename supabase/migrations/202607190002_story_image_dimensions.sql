...
begin
  ...
  loop
    if v_block ->> 'type' = 'text' then
      ...
      insert into public.story_blocks(
        story_id, block_type, storage_path, mime_type, image_width, image_height, position
      )
      values (
        v_story_id,
        'image',
        v_upload.object_path,
        v_upload.expected_mime_type,
        case when coalesce(v_block ->> 'width', '') ~ '^\d+$' then greatest(1, least((v_block ->> 'width')::integer, 4096)) else null end,
        case when coalesce(v_block ->> 'height', '') ~ '^\d+$' then greatest(1, least((v_block ->> 'height')::integer, 4096)) else null end,
        v_position
      );
      v_position := v_position + 1;
    else
